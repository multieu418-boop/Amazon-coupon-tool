    ...
    def parse_error_details(comment_text):
        ...
        if len(blocks) > 1:
            for i in range(1, len(blocks), 2):
                ...
                # 正则解析：使用非捕获组确保只匹配“要求”后面的数字
                # 匹配：要求的净价格/要求的最高商品价格/Required net price/Maximum product price allowed
...
with st.sidebar:
    ...
    if error_feedback_file and all_listing_file:
        ...
        reason_kw = st.text_input("报错原因关键字过滤")
...
with tab1:
    if not site_template:
        ...
        with st.form("gen_form"):
            ...
            for i, h in enumerate(headers):
                ...
                if "ASIN" in h_str.upper():
                    ...
                    user_input[h] = target_col.date_input(h, value=datetime.now(), key=f"d_{i}").strftime("%Y-%m-%d")
                else:
                    user_input[h] = target_col.text_input(h)
            
            if st.form_submit_button("🚀 生成初始提报文件"):
        ...
        if "gen_data" in st.session_state:
            st.download_button("📥 下载提报文件", st.session_state.gen_data, "Coupon_Upload_New.xlsx")

# --- 第二阶段逻辑 ---
with tab2:
    if not all_listing_file or not error_feedback_file or not site_template:
        st.warning("👈 请确保左侧上传了：1.空白模板 2.Listing报告 3.报错文件")
    else:
        # 数据解析 (仅在第一次上传时运行)
        if 'master_df' not in st.session_state:
            with st.spinner("正在精准匹配 ASIN 与价格数据..."):
                # 读取 Listing 报告
                ...
                for r_idx in range(10, ws_err.max_row + 1):
                    ...
                    for a in asins:
                        ...
                        p_col_name = next((c for c in df_l.columns if 'price' in c or '价格' in c), None)
                        p_match = df_l[df_l[asin_col_name] == a][p_col_name].values if asin_col_name else []
        ...
        if st.session_state.get('master_df') is not None:
            ...
            edited = st.data_editor(
                ...
                hide_index=True, use_container_width=True, key="fix_editor"
            )
            ...
            # --- 需求3：使用空白模板作为底稿导出 ---
            if st.button("🚀 生成纯净修复版 Excel (基于空白模板)", use_container_width=True):
                site_template.seek(0)
                wb_final = openpyxl.load_workbook(site_template)
                ws_final = wb_final.active
                
                # 重新获取报错文件的原始行数据（为了复制其他非 ASIN 列）
                error_feedback_file.seek(0)
                ...
                for (orig_line, disc), group in final_keep.groupby(['原始行号', '拟提报折扣']):
                    ...
                    for c_idx in range(1, len(f_headers) + 1):
                        ...
                        # 样式复制（从底稿第9行复制样式，确保纯净）
                        ref_style = ws_final.cell(row=9, column=c_idx)
                        if ref_style.has_style:
                            target_cell.font = copy(ref_style.font)
                            target_cell.border = copy(ref_style.border)
                            target_cell.fill = copy(ref_style.fill)
                            target_cell.alignment = copy(ref_style.alignment)
                ...
                st.session_state.fix_data = out_fix.getvalue()
                st.success("✅ 纯净修复文件已生成！(已移除亚马逊返回的末尾报错列)")

            if "fix_data" in st.session_state:
                st.download_button("📥 下载修复后的纯净 Excel", st.session_state.fix_data, "Fixed_Submission_Clean.xlsx")
