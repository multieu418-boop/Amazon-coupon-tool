    ...
    if not all_listing_file or not error_feedback_file:
        ...
        if 'master_df' not in st.session_state:
            with st.spinner("深度比对数据中..."):
                ...
                err_headers = [cell.value for cell in ws_err[7] if cell.value] # 只取有值的标题
                ...
                if not asin_col or not price_col or not e_asin_h:
                    st.error(f"❌ 无法识别关键列。Listing列：{asin_col}, {price_col} | 报错模板列：{e_asin_h}")
                    st.stop()

                rows = []
                # 建立列名到索引的映射，防止 KeyError
                ...
                for r_idx in range(10, ws_err.max_row + 1):
                    ...
                    comment_text = ws_err.cell(row=r_idx, column=ws_err.max_column).comment
                    comment_text = comment_text.text if comment_text else ""
                    ...
                    for a in asins:
                        p_match = df_l[df_l[asin_col] == a][price_col].values
                ...
                st.session_state.orig_err_headers = [cell.value for cell in ws_err[7]]
        ...
        if st.session_state.get('master_df') is not None:
            ...
            if st.button("🚀 生成并导出修复结果", use_container_width=True):
                error_feedback_file.seek(0)
                ...
                # 定位写入列
                headers_final = st.session_state.orig_err_headers
                a_idx = next((i for i, h in enumerate(headers_final, 1) if h and 'ASIN' in str(h)), 1)
                d_idx = next((i for i, h in enumerate(headers_final, 1) if h and '折扣' in str(h) and '数值' in str(h)), 3)
                ...
                out_fix = BytesIO()
                wb_out.save(out_fix)
                st.download_button("📥 下载修复后的 Excel", out_fix.getvalue(), "Fixed_Result.xlsx")
